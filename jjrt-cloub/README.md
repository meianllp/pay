...
### 开源用户登记，宣传用：[点击打开](https://gitee.com/geek_qi/jjrt-security/issues/II9SP)
### QQ群号：169824183
...
### 更新日志，查看[点击打开](https://gitee.com/geek_qi/jjrt-security/blob/master/README.md#%E5%BC%80%E6%BA%90%E7%89%88%E6%9B%B4%E6%96%B0%E6%97%A5%E5%BF%97)
...
### 运行步骤
- 运行数据库脚本：依次运行数据库：jjrt-admin/db/init.sql、jjrt-auth-server/db/init.sql、jjrt-trace
- 修改配置数据库配置：jjrt-admin/src/main/resources/application.yml、jjrt-gate/src/main/resources/application.yml
- 按`顺序`运行main类：CenterBootstrap（jjrt-center）、AuthBootstrap（jjrt-auth-server）、AdminBootstrap（jjrt-admin）、GatewayServerBootstrap（jjrt-gateway-v2）

### 项目结构
```
├─jjrt-cloub
│  │  
│  ├─jjrt-modules--------------公共服务模块（基础系统、搜索、OSS）
│  │ 
│  ├─jjrt-auth-----------------鉴权中心
│  │ 
│  ├─jjrt-gate-----------------网关负载中心
...
│  ├─jjrt-center---------------服务注册中心
│  │   
│  ├─jjrt-control--------------运维中心（监控、链路）
│  │
│  └─jjrt-sidebar--------------调用第三方语言
│
```
----
